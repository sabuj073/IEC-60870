...
void
printCP56Time2a(CP56Time2a time)    //print time stamp
{
...
static void
rawMessageHandler(void* parameter, IMasterConnection conneciton, uint8_t* msg, int msgSize, bool sent)  //log of sent or received messages
{
    if (sent)
        printf("SEND: ");
    else
...
/*On the server side we should use the InterrogationHandler callback function to handle the Interrogation request.
 Depending on the QOI (Qualifier of interrogation) value you should return different information objects.*/
...
interrogationHandler(void* parameter, IMasterConnection connection, CS101_ASDU asdu, uint8_t qoi)
{
    ...
    if (qoi == 20) { /* only handle station interrogation */
        ...
        CS101_AppLayerParameters alParams = IMasterConnection_getApplicationLayerParameters(connection); //to send the interrogated data back to the client/master

        IMasterConnection_sendACT_CON(connection, asdu, false);

        /* The CS101 specification only allows information objects without timestamp in GI responses */

        CS101_ASDU newAsdu = CS101_ASDU_create(alParams, false, CS101_COT_INTERROGATED_BY_STATION,  // To create a new ASDU object the CS101_ASDU_create function has to be used as a constructor of the object.
                0, 1, false, false);

        InformationObject io = (InformationObject) MeasuredValueScaled_create(NULL, 100, -1, IEC60870_QUALITY_GOOD);

        CS101_ASDU_addInformationObject(newAsdu, io);  //With this function we can add information object instances to the ASDU.
        ...
        //With this function we can add information object instances to the ASDU.
...
static bool
connectionRequestHandler(void* parameter, const char* ipAddress)   // check the client IP address against a whitelist of allowed clients or implement a blacklist.
{
...
static void
connectionEventHandler(void* parameter, IMasterConnection con, CS104_PeerConnectionEvent event)  //state of the connection
{
...
int
main(int argc, char** argv)   //driver or main function
{
    /* Add Ctrl-C handler */
    ...
    CS104_Slave slave = CS104_Slave_create(100, 100);  //creates a new slave

    CS104_Slave_setLocalAddress(slave, "0.0.0.0");    //the server is restricted to a single ip

    /* Set mode to a multiple redundancy groups
     * NOTE: library has to be compiled with CONFIG_CS104_SUPPORT_SERVER_MODE_SINGLE_REDUNDANCY_GROUP enabled (=1)
     */

     /* Server provide three different mode -
     The default mode-allows only a single active client connection
     The second mode - allows multiple active client connections.
     The third mode - allows multiple active client connections while preserving events when no client is connected.
    */

    CS104_Slave_setServerMode(slave, CS104_MODE_MULTIPLE_REDUNDANCY_GROUPS);  /* Server mode is selected*/

    CS104_RedundancyGroup redGroup1 = CS104_RedundancyGroup_create("red-group-1");  /*Redundancy groups only have to be created explicitly when using the server mode*/
...
//    //CS104_Slave_setRawMessageHandler(slave, rawMessageHandler, NULL);
    ...
    while (running) {
        ...
        CS101_ASDU newAsdu = CS101_ASDU_create(alParams, false, CS101_COT_PERIODIC, 0, 1, false, false); /*To create a new ASDU objectTo create a new ASDU object*/
